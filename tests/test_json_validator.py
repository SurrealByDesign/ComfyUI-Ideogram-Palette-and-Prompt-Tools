...
limits, hex validation, bbox bounds, and graceful failure."""
...
def test_valid_well_ordered_passes():
    out_json, is_valid, report = _node().validate(json.dumps(_well_ordered()), True, False)
...
def test_key_order_preserved_when_fix_disabled():
    ...
    out_json, _, _ = _node().validate(json.dumps(scrambled), False, False)
...
def test_invalid_json_does_not_crash():
    out_json, is_valid, report = _node().validate("{not valid json", True, False)
...
def test_global_palette_over_16_fails():
    ...
    out_json, is_valid, report = _node().validate(json.dumps(_well_ordered(color_palette=palette)), True, False)
    assert is_valid is False
...
def test_element_palette_over_5_fails():
    ...
    out_json, is_valid, report = _node().validate(json.dumps(_well_ordered(elements=elements)), True, False)
    assert is_valid is False
    assert "max 5" in report


def test_malformed_hex_is_reported():
    out_json, is_valid, report = _node().validate(
...
def test_missing_top_level_key_warns_but_can_pass():
    ...
    out_json, is_valid, report = _node().validate(json.dumps(minimal), True, False)
...
def test_content_values_unchanged():
    ...
    out_json, _, _ = _node().validate(json.dumps(original), True, False)
    parsed = json.loads(out_json)

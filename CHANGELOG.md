...
- `IdeogramElementBuilder` and `IdeogramElementCollector` nodes: build a single `compositional_deconstruction` element from a bounding box/description/type/optional per-element palette, then collect 1-8 of those into a complete block, skipping empty/malformed inputs and reporting overlapping **text** element bounding boxes. See README's Nodes section for the full reference.
- `IdeogramVisualPromptDesigner` node (Phase 1 of the planned Visual Prompt Designer), category `Ideogram/Palette` matching every other node in this package: builds a prompt from scratch or loads an existing one, reconstructs an editable internal "designer state," restores designer-only fields (subjects/hierarchy/relationships/notes/per-element depth-weight-lock-hide-group) from a previously saved state with depth_layer values validated against a known set (invalid restored values are rejected with a warning rather than accepted silently), applies an optional palette override, and re-validates with the existing `IdeogramJSONValidator` (reused, not reimplemented) -- all through standard widgets, with every warning/note printed to the console (matching `IdeogramElementBuilder`'s pattern) in addition to the `report` output. The drag/resize canvas, scene-graph tree, relationship graph, and undo/redo described in the full spec require a custom ComfyUI frontend extension and are a separate, future phase; see README's new "Ideogram Visual Prompt Designer" section for the full status table, architecture (including why all three text outputs are `STRING`-typed rather than the spec's distinct socket types), round-trip workflow, and a documented limitation (composition elements are matched between a freshly loaded prompt and saved designer state by position, not a persistent ID -- a position-derived id was deliberately not added, since it would be exactly as fragile as the index it's derived from). No `reference_image` input is exposed -- an earlier draft had one that accepted an image and did nothing with it; that was removed rather than kept as a placeholder, since an input with no effect implies a capability that doesn't exist. Verified end-to-end against a live ComfyUI server, including the round-trip workflow and the depth_layer rejection path.
- `utils/designer_model.py`: the Visual Prompt Designer's Layer 1 (visual model) / Layer 3 (serialization) module -- pure stdlib, reuses `schema_utils` for key ordering and hex validation.
- `IdeogramJSONValidator` gained two new outputs, `errors_json` and `warnings_json` (JSON arrays of message strings), appended after the existing three outputs so saved workflows' connections to `prompt_json`/`is_valid`/`report` are unaffected. Added so `IdeogramVisualPromptDesigner`'s `designer_state.validation` field could carry structured `errors`/`warnings` lists instead of only an unstructured report string. The three showcase workflows that use this node (`showcase_01_palette_to_prompt_pipeline.json`, `showcase_02_blend_content_and_style.json`, `showcase_05_embed_recover_roundtrip.json`) had their saved node definitions updated to match, with both new output slots left unconnected (matching how a resave from the ComfyUI UI would represent them).

### Fixed
- `IdeogramJSONValidator` dropped `ensure_ascii=False` when re-serializing with `fix_key_order=True` (the default), silently rewriting non-ASCII characters into `\uXXXX` escape sequences on every call that fixed key order -- inconsistent with every other JSON-producing node in this repo, which all preserve literal UTF-8. Found while wiring `IdeogramVisualPromptDesigner` (the first caller to chain non-ASCII content through the validator with key-order fixing on) and confirmed against a live ComfyUI run before and after the fix.
- The same missing-`ensure_ascii=False` bug also existed in `IdeogramPromptAssembler` (both `json.dumps` call sites in `assemble()`) and `IdeogramPaletteToGlobalJSON` (`build()`'s return) -- same root cause, same fix, found by auditing every other `json.dumps` call in the package once the validator instance turned up. A fourth instance of the same bug was introduced by this very audit pass, in the new `errors_json`/`warnings_json` outputs added to `IdeogramJSONValidator` above (a malformed-hex warning can itself contain non-ASCII text via its `!r`-quoted color value) -- fixed in the same pass it was added, before it shipped. Added regression tests for all four call sites.
